...
class MysqliDb
{
    ...
    public function __construct($host = NULL, $username = NULL, $password = NULL, $db = NULL, $port = NULL, $charset = 'utf8')
    {
        ...
        if (isset ($prefix)) $this->setPrefix ($prefix);
        self::$_instance = $this;
    }
    ...
    public function connect()
    {
		...
		else if(empty ( $this->host ))  return null ; // Mysql host is not set ;
		...
		if( $this->_mysqli->connect_error ) return null ; // throw new Exception ('Connect Error ' . $this->_mysqli->connect_errno . ': ' . $this->_mysqli->connect_error);
		else if($this->charset) $this->_mysqli->set_charset ($this->charset);
    ...
    public function get($tableName, $numRows = null, $columns = '*')
    {
        ...
        if (strpos ($tableName, '.') === false) $this->_tableName = self::$prefix . $tableName;
